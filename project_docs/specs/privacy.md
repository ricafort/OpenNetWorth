...
### Requirement: Admin "Break-Glass" Access
The system SHALL strictly prohibit admin access to sensitive user data unless explicit consent is granted.

#### Scenario: Support Request
- **GIVEN** user has a technical issue
- **WHEN** user grants "24h Support Access" via settings
- **THEN** admin can query user's assets/liabilities for 24 hours
- **AND** admin dashboard verifies active grant before fetching

### Privacy Architecture Diagram

```text
[ 👤 User ] ──(1. Grant Access)──▶ [ 🛡️ admin_access_grants ] ◀──(3. Verify Grant)──┐
                                          (Active for 24h)                          │
                                                                                    ▼
[ 👮 Admin ] ──(2. View Dashboard)──────────────────────────────────────────▶ [ 🔒 RLS Policy ]
                                                                                    │
                                                                       ┌────────────┴────────────┐
                                                                       ▼                         ▼
                                                              [ ✅ Allow (Grant Exists) ] [ ❌ Deny (Default) ]
```
