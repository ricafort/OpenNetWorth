...
import { toMonthlyAmount } from '@/infrastructure/local_driver';
...
export default function RecurringList({ transactions, onEdit, onDelete, onToggle, currencyCode }: Props) {
    const income = transactions.filter(t => t.type === 'income');
    const expenses = transactions.filter(t => t.type === 'expense');

    // Calculate totals - Convert EACH item first, then sum
    // This ensures consistency with individual item display
    const calculateTotal = (items: RecurringTransaction[]) => {
        return items
    ...
    const totalRecurringIncome = calculateTotal(income);
    const totalRecurringExpenses = calculateTotal(expenses);
        ...
        <div className="bg-muted rounded-2xl p-4 md:p-6 border border-border">
            <div className="flex items-center justify-between mb-4">
                ...
                <span className={`font-black font-mono ${colorClass} privacy-value`}>
                    {formatCurrency(total, currencyCode)}/mo
                </span>
            </div>
            ...
            <div className="space-y-3">
                {items.length === 0 ? (
                    ...
                    items.map(t => {
                        ...
                        return (
                            <div key={t.id} className={`flex items-center gap-3 p-3 bg-white rounded-xl border transition-all ${t.is_active ? 'border-slate-200 shadow-sm' : 'border-border opacity-60 grayscale'}`}>
                                {/* Toggle */}
                                <button
                                    onClick={() => onToggle(t.id)}
                                    className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${t.is_active ? 'bg-blue-100 text-blue-600' : 'bg-slate-100 text-slate-400'}`}
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className="font-bold text-slate-900">{formatCurrency(amountConverted, currencyCode)}</span>
                                    </div>
                                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                                        <span className="capitalize">{t.frequency} • {t.category}</span>
                                        {t.frequency !== 'monthly' && (
                                            <span className="text-slate-400">
                                                ({formatCurrency(monthlyConverted, currencyCode)}/mo)
                                            </span>
                                        )}
                                    </div>
                                </div>
